...
if (typeof window !== 'undefined') {
  ...
  // Compute and display the final TOTAL INVESTMENT amount by summing all checked pills in the table
  const updateTotalInvestmentDisplays = () => {
    try {
      ...
      for (const t of tables) {
        const totalRow = Array.from(t.querySelectorAll('tr')).find(r => /TOTAL\s+INVESTMENT/i.test(r.textContent || '')) as HTMLTableRowElement | undefined;
        if (!totalRow) continue;
        const cells = Array.from(totalRow.querySelectorAll('td,th')) as HTMLTableCellElement[];
        if (!cells.length) continue;
        const labelCell = cells[0];
        const inputs = Array.from(t.querySelectorAll('input.proposal-price-checkbox')) as HTMLInputElement[];
        ...
        let finalSum = sum;
        if (!(isFinite(finalSum) && finalSum > 0)) {
          const singlePill = inputs.length === 1 ? Number(inputs[0]?.getAttribute('data-amount') || '0') : 0;
          if (isFinite(singlePill) && singlePill > 0) finalSum = singlePill;
        }
        let disp = labelCell.querySelector('.trim-total-amount') as HTMLElement | null;
        if (!disp) {
          disp = document.createElement('span');
          disp.className = 'trim-total-amount';
          disp.style.fontWeight = 'bold';
          disp.style.marginLeft = '4px';
          ...
          if (anchor) labelCell.insertBefore(disp, anchor.nextSibling); else labelCell.appendChild(disp);
        }
        disp.textContent = fmtUSD(finalSum) || '';
      }
    } catch {}
  };
  ...
  const fixRoofingTotals = () => {
    ...
    for (const t of tables) {
      ...
      if (!isAsphalt && !isDavinci) continue;
      ...
      // 2) Asphalt collapsed (Case A): no GBB cells => prefer placing G/B/B pills in the 'Please Check Selection' prompt row
      if (isAsphalt && gbbCells.length === 0) {
        const tiers = getAsphaltTiers();
        // Find the prompt row either via data-hidden-prompt or text content
        const promptRow = Array.from(t.querySelectorAll('tr')).find(r =>
          (r.getAttribute('data-hidden-prompt') === '1') || /PLEASE\s+CHECK\s+SELECTION/i.test(r.textContent || '')
        ) as HTMLTableRowElement | undefined;
        if (promptRow) {
          // Unhide the row
          (promptRow as HTMLElement).style.display = 'table-row';
          (promptRow as HTMLElement).setAttribute('data-force-visible', '1');
          // Cells: [0]=label, [1]=Good, [2]=Better, [3]=Best
          const pcs = Array.from(promptRow.querySelectorAll('td,th')) as HTMLTableCellElement[];
          const goodCell = pcs[1]; const betterCell = pcs[2]; const bestCell = pcs[3];
          if (goodCell) { goodCell.setAttribute('data-gbb', 'good'); goodCell.style.textAlign = 'center'; goodCell.querySelectorAll('label.price-choice').forEach(el => el.remove()); goodCell.appendChild(mkPill(tiers.good, 'gbb')); }
          if (betterCell) { betterCell.setAttribute('data-gbb', 'better'); betterCell.style.textAlign = 'center'; betterCell.querySelectorAll('label.price-choice').forEach(el => el.remove()); betterCell.appendChild(mkPill(tiers.better, 'gbb')); }
          if (bestCell) { bestCell.setAttribute('data-gbb', 'best'); bestCell.style.textAlign = 'center'; bestCell.querySelectorAll('label.price-choice').forEach(el => el.remove()); bestCell.appendChild(mkPill(tiers.best, 'gbb')); }
          // If a collapsed TOTAL single-cell row exists, remove it to avoid confusion
          const rowCells = Array.from(totalRow.querySelectorAll('td,th')) as HTMLTableCellElement[];
          const isSingleCellTotal = rowCells.length === 1 && ((rowCells[0].colSpan || 1) > 1);
          if (isSingleCellTotal) totalRow.remove();
          continue;
        }
        // If prompt row is not found, retain previous behavior
        const rowCells = Array.from(totalRow.querySelectorAll('td,th')) as HTMLTableCellElement[];
        const isSingleCellTotal = rowCells.length === 1 && ((rowCells[0].colSpan || 1) > 1);
        if (isSingleCellTotal) {
          ...
          const parent = totalRow.parentElement; if (parent) parent.replaceChild(newRow, totalRow);
          continue;
        } else {
          labelCell.querySelectorAll('label.price-choice').forEach(el => el.remove());
      ...
      // 3) DaVinci (Cases C/D): keep a single pill inline after label; remove centered/duplicate pills.
      if (isDavinci) {
        ...
        labelCell.querySelectorAll('label.price-choice').forEach(el => el.remove());
        const dv = getDavinciTotal();
        const pill = mkPill(dv);
  ...
  const watch = new MutationObserver(() => {
    ...
    // Reinforce Case A prompt row pills if they were removed by later mutations
    const tables = Array.from(document.querySelectorAll('table')) as HTMLTableElement[];
    tables.forEach(t => {
      const text = (t.textContent || '').toUpperCase();
      const isAsphalt = /(ASPHALT|SHINGLE|NORTHGATE|CLIMATEFLEX)/.test(text);
      ...
      const pcs = Array.from(promptRow.querySelectorAll('td,th')) as HTMLTableCellElement[];
      const tiers = getAsphaltTiers();
      const ensurePill = (cell: HTMLTableCellElement | undefined, key: string, amt: number) => {
        if (!cell) return;
        cell.setAttribute('data-gbb', key);
        cell.style.textAlign = 'center';
        if (!cell.querySelector('label.price-choice')) cell.appendChild(mkPill(amt, 'gbb'));
      };
      (promptRow as HTMLElement).style.display = 'table-row';
      (promptRow as HTMLElement).setAttribute('data-force-visible', '1');
      ensurePill(pcs[1], 'good', tiers.good);
      ensurePill(pcs[2], 'better', tiers.better);
      ensurePill(pcs[3], 'best', tiers.best);
    });
  });
...
export default function ProposalView({ params }: { params: { id: string } }) {
  ...
  // One-time builder for Asphalt Case A: synthesize GBB TOTAL row
  try { if (root && html) buildAsphaltGbbIfMissing(root); } catch {}
  ...
  // Normalize roofing total pills across materials before generic wrappers
          ...
          {proposal?.signedAt ? (
            ...
            <div className="mt-4">
              ...
              <div className="flex items-center gap-2 mt-3">
                ...
                <button
                  className={`px-4 py-2 rounded font-medium transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed ${submitting ? 'bg-green-500 text-white cursor-wait' : 'bg-green-600 hover:bg-green-700 text-white'}`}
                  onClick={submit}
                  disabled={!name || submitting}
                >{submitting ? 'Submitting…' : 'Accept and Submit'}</button>
              </div>
              {submitting && (
                <div className="mt-3 h-2 w-full rounded bg-slate-200 overflow-hidden" aria-live="polite" aria-label="Submitting proposal">
