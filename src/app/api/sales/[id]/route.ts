...
async function updateCommission(req: NextRequest, { id }: { id: string }){
  ...
  const body = await req.json().catch(()=>({} as any)) as { commissionPercent?: number|null };
  ...
  // base pay fields removed

    if (Object.keys(data).length === 0) return NextResponse.json({ ok:false, error:'No valid fields to update' }, { status: 400 });

    const updated = await prisma.user.update({ where: { id }, data });
  return NextResponse.json({ ok:true, item: { id: updated.id, commissionPercent: updated.commissionPercent } });
  } catch (e:any) {
